...
class C2f(nn.Module):
    def __init__(self,in_channels, out_channels, num_bottlenecks,shortcut=True):
        ...
        self.conv1=Conv(in_channels,out_channels, kernel_size=1,stride=1, padding=1)
        #sequence of bottleneck lyers
        self.m=nn.ModuleList(
            [Bottleneck(self.mid_channels,self.mid_channels) for _ in range(num_bottlenecks)])
        self.conv2=Conv((num_bottlenecks+2)*out_channels//2,out_channels,kernel_size=1,stride=1, padding=0)
    def forward(self,x):
        x=self.conv1(x)

        x1,x2=x[:,:x.shape[1]//2,:,:],x[:,x.shape[1]//2:,:,:]
        outputs=[x1,x2]    #x1 to bottle neck
        for i in range(self.num_bottlenecks):
            x1=self.m[i](x1) #[bs,0.5c_out,w,h]
            outputs.insert(0,x1)
        outputs=torch.cat(outputs,dim=1)#[bx,0.5c_out(num_bottleneck+2),w,h]
...
class Backbone(nn.Module):
    def __init__(self,version, in_channels=3,shortcut=True):
        ...
        self_conv_5=Conv(int(256*w),int(512*w), kernel_size=3, stride=2, padding=1)
